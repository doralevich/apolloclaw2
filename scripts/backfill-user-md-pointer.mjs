#!/usr/bin/env node
/**
 * backfill-user-md-pointer.mjs — teach already-running agents to read their USER.md.
 *
 * New and re-submitted setups get this automatically (lib/provision.ts appends the pointer
 * right after the USER.md write). Agents provisioned before that shipped have the file but
 * no instruction to open it, so they answer "I don't know who you are" while a full profile
 * of their owner sits in the next file over. This walks those instances once.
 *
 *   AGENT37_API_KEY=... node scripts/backfill-user-md-pointer.mjs [--dry-run] [instanceId...]
 *
 * With no instance IDs it visits every instance the key can see. Idempotent: the marker is
 * checked inside the instance, so re-running is a no-op and instances that never received a
 * USER.md are skipped rather than given a pointer to a file that isn't there.
 *
 * KEEP THE POINTER TEXT BELOW IN SYNC WITH lib/provision.ts (source of truth).
 */

import { readFileSync } from "fs";
import { resolve, dirname } from "path";
import { fileURLToPath } from "url";

const MARKER = "<!-- apollo:user-md-pointer -->";

const POINTER = `

${MARKER}
## Who you work for

There is a USER.md beside this file in your workspace. It holds your owner's answers from
their setup questionnaire: who they are, their business, how it runs, and where it hurts.

Read it at the start of every session, before your first reply. Treat it as ground truth
about them the way you treat this file as ground truth about yourself, and keep it current
as you learn more. Never tell your owner you don't know who they are — you do, it is written
down, go and read it.
`;

// ── Load .env.local (same pattern as seed-stripe-catalog.mjs) ─────────────────
const here = dirname(fileURLToPath(import.meta.url));
try {
  const raw = readFileSync(resolve(here, "..", ".env.local"), "utf8");
  for (const line of raw.split("\n")) {
    const m = line.match(/^\s*([A-Z0-9_]+)\s*=\s*(.*)\s*$/);
    if (m && !process.env[m[1]]) process.env[m[1]] = m[2].replace(/^["']|["']$/g, "");
  }
} catch {
  // no .env.local — rely on the ambient environment
}

const KEY = process.env.AGENT37_API_KEY;
const BASE = (process.env.AGENT37_API_BASE_URL || "https://api.agent37.com").replace(/\/$/, "");
if (!KEY) {
  console.error("AGENT37_API_KEY is not set (env or .env.local).");
  process.exit(1);
}

const args = process.argv.slice(2);
const dryRun = args.includes("--dry-run");
const ids = args.filter((a) => !a.startsWith("--"));

async function api(path, init) {
  const res = await fetch(`${BASE}${path}`, {
    ...init,
    headers: { Authorization: `Bearer ${KEY}`, "Content-Type": "application/json", ...(init?.headers || {}) },
  });
  const body = await res.json().catch(() => ({}));
  if (!res.ok) {
    const msg = body?.error?.message || body?.message || res.statusText;
    throw new Error(`${res.status} ${msg}`);
  }
  return body;
}

// Only touch instances that actually have a USER.md, and only once. Everything is decided
// inside the container so a half-provisioned box can't be given a dangling pointer.
const b64 = Buffer.from(POINTER, "utf8").toString("base64");
const CMD =
  'WS="${OPENCLAW_STATE_DIR:-/home/node/.openclaw}/workspace"; ' +
  '[ -f "$WS/USER.md" ] || { echo NO_USER_MD; exit 0; }; ' +
  'F="$WS/SOUL.md"; touch "$F"; ' +
  `grep -qF '${MARKER}' "$F" && { echo ALREADY; exit 0; }; ` +
  `printf '%s' '${b64}' | base64 -d >> "$F"; echo POINTER_OK`;

const targets = ids.length
  ? ids.map((id) => ({ id, name: id }))
  : (await api("/instances")).data.map((i) => ({ id: i.id, name: i.name || i.id }));

console.log(`${dryRun ? "[dry run] " : ""}${targets.length} instance(s) to visit\n`);

let added = 0;
let skipped = 0;
let failed = 0;

for (const t of targets) {
  if (dryRun) {
    console.log(`  would visit  ${t.name} (${t.id})`);
    continue;
  }
  try {
    const { stdout } = await api(`/instances/${t.id}/exec`, {
      method: "POST",
      body: JSON.stringify({ command: CMD }),
    });
    if (stdout.includes("POINTER_OK")) {
      added++;
      console.log(`  pointer added  ${t.name} (${t.id})`);
    } else if (stdout.includes("ALREADY")) {
      skipped++;
      console.log(`  already had it ${t.name}`);
    } else {
      skipped++;
      console.log(`  no USER.md     ${t.name}`);
    }
  } catch (err) {
    failed++;
    console.error(`  FAILED         ${t.name} (${t.id}): ${err.message}`);
  }
}

if (!dryRun) {
  console.log(`\nadded ${added}, skipped ${skipped}, failed ${failed}`);
}
process.exit(failed > 0 ? 1 : 0);
