...
// The CFO Agent's intake, on an unlisted link. Same white-glove model as /white-glove-onboarding
// (no paywall, no login, files a lead) but pinned to the `cfo` agent type, so the questionnaire
// adds the finance deep-dive (lib/cfoIntake.ts) on top of the standard business questions.
//
// David hands this URL out directly to a CFO prospect; it is not linked from anywhere on the site
// and is noindex/nofollow via the layout, so the only way in is the URL itself. The intake is
// filed as a lead (CRM email + PDF, with a "CFO Deep-Dive" section), and David provisions the
// CFO agent from it.
//
// CLOSING THE DEAL. Append ?pay=<Stripe Payment Link> and the finish screen ends on a payment
// hand-off, so a prospect fills the CFO intake and then pays whatever was agreed - the same
// custom, per-engagement pricing the /ai-agents/cfo page sells on.
...
export default function CfoOnboardingPage() {
  ...
  return <OnboardingForm mode="whiteglove" agentTypeId="cfo" payUrl={payUrl} />;
}
