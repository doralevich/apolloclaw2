...
function authorized(req: NextRequest) {
  const token = process.env.ZOOM_STATUS_TOKEN;
  if (!token) return false;
  return req.headers.get("x-zoom-status-token") === token;
}
...
export async function GET(req: NextRequest) {
  ...
  const env = {
    ...
    ZOOM_STATUS_TOKEN: Boolean(process.env.ZOOM_STATUS_TOKEN),
  };

  try {
    await ensureZoomStorageLayout();
  } catch {
    // Continue reporting partial status. Do not expose error internals here.
  }
