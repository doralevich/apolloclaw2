...
// so Composio returns the user to this tab when they finish (?composioConnect=1&toolkit=…). The
// v1 route requires it be absolute https, so on a local http origin we omit it (the connect still
// works; the user just lands on Composio's default page instead of back here).
export const POST = route(async (request: Request, { params }: Ctx) => {
  ...
  try {
    ...
    // Some apps need a bring-your-own-credentials flow we don't expose here; the v1
