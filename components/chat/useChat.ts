...
export function useChat({ agentId, sessionId, onSessionCreated, onActivity }: UseChatArgs) {
  ...
  useEffect(() => {
    ...
    if (!sessionId) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reset-on-prop-change: clearing the transcript when the thread switches to New Chat is exactly this effect's job
      setMessages([]);
      return;
    }
  ...
  const send = useCallback(
    async (text: string, settings: SendSettings = {}) => {
      ...
      try {
        ...
        // Single sweep on EVERY exit path (clean end, Stop/abort, stream error) so a tool chip
        // can never spin forever; covers dropped/unpaired completed events too.
        finalizeTools();
